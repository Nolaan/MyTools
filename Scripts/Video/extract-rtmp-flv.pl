...
our $debug_ascii_in       = 0;		# True to assume tcpdump is in ASCII
...
our $def_rtmp_chunk_size  = 4096;	# Default RTMP chunk size
...
while (! $datastream_eof) {
    ...
    #if (($cur_routing_id > $max_rtmp_routing_id)) {
        $cur_routing_id = 0;
    #}
